...
class IntegrityRule(Rule):
    ...
    def evaluate(self, event, data):
        ...
        if tampered or missing:
            create_tamper_marker()
            lines = ["\n  \U0001f6a8 HOOK INTEGRITY ALERT \U0001f6a8"]
            if tampered:
                lines.append(f"  TAMPERED: {', '.join(tampered)}")
            if missing:
                lines.append(f"  MISSING:  {', '.join(missing)}")
            lines.append("  Run: sudo python3 ~/.copilot/tools/install.py --lock-hooks\n")
            return info("\n".join(lines))

        # All good — clear tamper marker
