...
This repo is a **set of standalone Python CLI scripts** — not a package or library. Each script is independently runnable and normally duplicates its own constants. Documented helper exceptions such as `_tentacle_core.py`, `_tentacle_goal.py`, and `_tentacle_pr.py` are allowed only when they preserve an existing CLI/module contract and are listed in the script inventory. This keeps the default goal operator-first simplicity, not framework cohesion.
...
| `tentacle.py` | Multi-agent orchestration (create → todo → bundle → swarm → complete) + orchestrator goal loop (`goal init/validate/status/dispatch/link/eval/resume/criteria/gate/budget/next-iter/verify-loop/coverage/context`). `goal.json` keeps both the legacy flat `tentacles` list and a structured per-iteration `iterations` map so status/history queries can answer which tentacles belonged to each iteration without breaking older readers. Goal writes are serialized with a `goal.json.lock` sidecar (`O_CREAT | O_EXCL`, 30 s timeout, PID-aware stale-lock cleanup), goal text uses a 3000-char warning / 5000-char hard limit with `goal validate` plus init-time enforcement, and `goal dispatch` reads tentacle `todo_deps` metadata to build a concurrency-limited ready/deferred plan. `goal eval --decision continue|complete` now blocks until every current-iteration tentacle has either a terminal handoff or a completed state. Budget enforcement: `goal eval --decision continue` when over any budget limit transitions the goal to `budget_limited` status and persists the reason in `goal.json`; further eval is blocked until `goal resume` is called after adjusting limits with `goal budget`. Bridge link tracking: `handoff … --bridge <sc-id>` (repeatable) writes a `Bridge:` line to `handoff.md` and validates the ID against `goal.json` criteria (fail-open); `goal coverage` reads `bridge_links` from completed tentacle `meta.json` files and reports covered/uncovered criteria plus orphan bridge IDs. `goal context` renders a compact continuation-context block (objective, iteration, budget, progress, remaining criteria, prior handoff summaries) suitable for injection into the next agent wave; `--write` persists it to `.octogent/goal-context.md`, and `goal eval --decision continue` / `goal resume` both auto-write the artifact. `sk tentacle goal …` routes here via Rust pass-through — no Rust code change is needed when adding new `goal` subcommands. |
