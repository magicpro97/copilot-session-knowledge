...
| `tentacle.py` | Multi-agent orchestration (create → todo → bundle → swarm → complete) + orchestrator goal loop (`goal init/validate/status/link/eval/resume/criteria/gate/budget/next-iter/verify-loop`). `goal.json` now keeps both the legacy flat `tentacles` list and a structured per-iteration `iterations` map so status/history queries can answer which tentacles belonged to each iteration without breaking older readers. Goal writes are serialized with a `goal.json.lock` sidecar (`O_CREAT | O_EXCL`, 30 s timeout, PID-aware stale-lock cleanup), and goal text uses a 3000-char warning / 5000-char hard limit with `goal validate` plus init-time enforcement. `sk tentacle goal …` routes here via Rust pass-through — no Rust code change is needed when adding new `goal` subcommands. |
