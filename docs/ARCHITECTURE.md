...
This repo is a **set of standalone Python CLI scripts** — not a package or library. Each script is independently runnable and normally duplicates its own constants. Documented helper exceptions such as `_tentacle_core.py` are allowed only when they preserve an existing CLI/module contract and are listed in the script inventory. This keeps the default goal operator-first simplicity, not framework cohesion.
