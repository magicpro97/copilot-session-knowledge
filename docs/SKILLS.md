...
**Global auto-update for supported built-in skills:** if `~/.copilot/skills/<skill-name>/SKILL.md` already exists (placed there manually), auto-update keeps it current whenever the source under `skills/<skill-name>/` changes. This is **update-only** — it never creates a new global install from scratch. For non-vendored built-ins such as `skill-creator`, auto-update also backfills missing bundled resources under the existing global skill directory so scripts, references, assets, and viewers stay in sync. This applies to **Copilot CLI scope only** (`~/.copilot/skills/`); a global `~/.claude/skills/` directory is not auto-updated. On WSL, auto-update also refreshes the current Windows user's Copilot CLI global install when that profile can be resolved, but that Windows-side install must already exist from a separate manual copy.
