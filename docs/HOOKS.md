...
| `skill-usage` | postToolUse | Records event-level skill usage in `skill-metrics.db` (`skill_usage_events` table) each time the `skill` tool fires. Three events per invocation: `triggered` (always), then `loaded` or `skipped` based on the tool result (exit code first; short-output skip-marker fallback). Exit code 0 always classifies as `loaded`; non-zero as `skipped`. Fail-open; never blocks. Event counts are included in `skill-metrics.py` default output (and `--json` status); the browse `/api/skills/metrics` endpoint returns `event_skill_usage` inline. Briefing uses the table to surface top/bottom skills by load rate. (issue #119) |
