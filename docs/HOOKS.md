...
hooks/
  ...
  rules/
    ...
    confidence_gate.py    # Blocks writes/closeout while research_gate is open
...
| `workflow-state` | sessionStart + preToolUse + postToolUse + sessionEnd | Tracks session workflow phase (idle→clarify→plan→execute→verify→close). Informational/advisory only — never denies. Emits phase-change notifications. State stored in `~/.copilot/markers/workflow-state-{session_id}.json`. Fail-open. (#664) |
...
| `compact-lifecycle` | preCompact + postCompact | Pauses active goal on preCompact; returns context restoration hint on postCompact. Fail-open. (Issue #665) |
...
| `confidence-gate` | preToolUse | Blocks edit/create, source-writing bash, `task_complete`, `git push/merge/commit`, and `gh pr create/merge` while `.github/conductor/last-plan.json`, `.copilot/confidence-gate.json`, or `~/.copilot/markers/confidence-gate.json` contains `research_gate.required=true`. The gate closes only when conductor reruns with confidence `1.0` (no `research_gate`) or an explicit override marker is recorded. See [CONFIDENCE-GATE.md](CONFIDENCE-GATE.md). |
...
| `skill-nudge` | postToolUse | Emits a one-time informational nudge toward skill creation after `SKILL_NUDGE_THRESHOLD` (default: `5`) tool calls in the current session. Uses shared per-session state (`update_session_state()`) so it fires exactly once per session. Informational-only. Fail-open. (issue #116) |
| `skill-usage` | postToolUse | Records event-level skill usage in `skill-metrics.db` (`skill_usage_events` table) each time the `skill` tool fires. Two events per invocation: `triggered` (always), then `loaded` or `skipped` based on the tool result (exit code first; short-output skip-marker fallback). Exit code 0 always classifies as `loaded`; non-zero as `skipped`. Absent or empty toolResult is fail-open `loaded`. Fail-open; never blocks. Event counts are included in `skill-metrics.py` default output (and `--json` status); the browse `/api/skills/metrics` endpoint returns `event_skill_usage` inline. Briefing uses the table to surface top/bottom skills by load rate. (issue #119) |
...
### Ported to native Rust (`sk-rust/src/hooks/rules/`)
...
| `learn-reminder` | postToolUse | ✅ Native direct path (wave6) | Emits the task-complete reminder, writes `learn-done` on `learn.py` and `sk learn` bash runs, and nudges skill-creator-standard updates for reusable lessons |
...
`sk-rust/src/hooks/rules/guard.rs`, registered between `SubagentGitGuardRule` and `BlockEditDistRule`.
...
Windows proof accepted: `cargo test --quiet` (unit tests in `rules/`) and
