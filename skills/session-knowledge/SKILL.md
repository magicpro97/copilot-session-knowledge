...
Knowledge confidence is not the same as decision confidence. If the relevant memory entry, the
briefing result, or your current task decision is below confidence `1.0`, do not act from memory
alone. Split the uncertainty into focused questions and dispatch independent research/validation
on the strongest available model (`claude-opus-4.7` when available; otherwise newest opus-class)
before implementation, deletion, merge, or routing decisions. After resolving the gap, record the
new evidence with `sk learn` so future briefings can reach confidence `1.0` faster.
