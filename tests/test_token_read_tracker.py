...
try:
    ...
    result_bash = rule.evaluate("postToolUse", {
        "toolName": "bash",
...
try:
    ...
    try:
        ...
        result_no_dup = rule2.evaluate("postToolUse", {
            "toolName": "view",
            "toolInput": {"path": str(_small_file)},
        })
        ...
        result_95 = rule2.evaluate("postToolUse", {
            "toolName": "view",
            "toolInput": {"path": str(_small_file)},
        })
...
try:
    ...
    _common.get_session_id = _orig_session_id
    _common.get_session_state_path = _orig_state_path
    _rt_mod.load_session_state = _common.load_session_state.__wrapped__ if hasattr(_common.load_session_state, "__wrapped__") else _common.load_session_state
...
try:
    ...
    _e2e_state_file.write_text(json.dumps({
        "files_read": {str(_real_file): {"count": 1, "tokens": 500, "first_read": 0}},
        "total_tokens": 500,
        "thresholds_warned": [],
    }), encoding="utf-8")
    _e2e_env2 = {**_e2e_env, "COPILOT_AGENT_SESSION_ID": str(os.getpid())}
    r10c = _run("preToolUse", {"toolName": "view", "toolArgs": {"path": str(_real_file)}}, env=_e2e_env2)
    test("10c: seeded repeat read → info message emitted on stderr",
         "already read" in r10c.stderr,
         f"stdout={r10c.stdout[:200]} stderr={r10c.stderr[:200]}")
    test("10c: info message NOT on stdout (preToolUse JSON channel clean)",
    ...
    _e2e_state_file80 = _e2e_markers / f"session-state-budget80"
    _small_view = REPO / "hooks" / "hooks.json"  # small file guaranteed to exist
    _e2e_state_file80.write_text(json.dumps({
        "files_read": {},
        "total_tokens": 799,
        "thresholds_warned": [],
    }), encoding="utf-8")
    ...
    test("10e: 80 % budget hit → TOKEN BUDGET WARNING in output",
...
try:
    ...
    try:
        ...
        result_jump = rule_jump.evaluate("postToolUse", {
            "toolName": "view",
        ...
        result_no_80 = rule_jump.evaluate("postToolUse", {
            "toolName": "view",
            "toolInput": {"path": str(_jump_file)},
        })
        if result_no_80 is not None:
            msg_no_80 = result_no_80.get("message", "")
            test("11c: subsequent call does NOT re-emit 80% warning", "\U0001f7e1" not in msg_no_80,
                 f"got: {msg_no_80[:100]}")
        else:
...
try:
    ...
    try:
        r12_default = read_rule_empty.evaluate("preToolUse", {
            "toolName": "view",
        ...
        r12_empty = read_rule_empty.evaluate("preToolUse", {
            "toolName": "view",
            "toolInput": {"path": _LOCK_PATH_EMPTY},
        })
        test("12b: .lock with empty env var → warning returned", r12_empty is not None,
...
try:
    ...
    test("14b: COPILOT_SESSION_ID wins when agent-id absent",
         _common14.get_session_id() == "session-id-wins")
    ...
    test("14c: basename(COPILOT_SESSION_STATE) wins when both above absent",
    ...
    test("14d: fallback is NOT the shared constant 'default-session'",
         fallback != "default-session",
         f"got: {fallback!r}")
    test("14d: fallback is not a bare numeric PID string",
         not fallback.isdigit(),
         f"got: {fallback!r}")
    test("14d: fallback is stable across two calls (same process)",
         _common14.get_session_id() == fallback,
         f"first={fallback!r} second={_common14.get_session_id()!r}")
    test("14d: fallback produces same marker suffix (write == cleanup alignment)",
    ...
    test("14e: distinct COPILOT_SESSION_ID → distinct marker paths",
    ...
    test("14f: distinct COPILOT_SESSION_STATE basenames → distinct marker paths",
         path_foo != path_bar,
...
try:
    ...
    test("15a: briefing suffix matches common suffix", suffix_briefing == suffix_common,
         f"briefing={suffix_briefing!r} common={suffix_common!r}")
    test("15a: session_lifecycle suffix matches common suffix", suffix_slc == suffix_common,
         f"slc={suffix_slc!r} common={suffix_common!r}")
    test("15a: all suffixes equal 'sess-abc123' (no sanitization needed)",
         suffix_common == "sess-abc123", f"got: {suffix_common!r}")
    ...
    test("15b: sanitize_session_id removes '/' chars", "/" not in s15b,
         f"got: {s15b!r}")
    test("15b: sanitize_session_id removes '\\\\' chars", "\\" not in s15b,
         f"got: {s15b!r}")
    ...
    try:
        ...
        try:
            ...
            no_traversal = False
        test("15b: marker path with traversal-ish session ID stays in MARKERS_DIR",
             no_traversal, f"marker={resolved_marker} dir={resolved_markers_dir}")
    finally:
    ...
    test("15c: '..' sequences in session ID are collapsed to '.'",
         ".." not in s15c_dot, f"got: {s15c_dot!r}")

    # ── 15d: Absolute path override prevented (session ID starting with '/') ─
    s15d = _common15.sanitize_session_id("/absolute/path/id")
    test("15d: leading '/' in session ID is sanitized", not s15d.startswith("/"),
         f"got: {s15d!r}")
    test("15d: no '/' remaining after sanitization of absolute path", "/" not in s15d,
         f"got: {s15d!r}")

    # ── 15e: Null bytes are stripped ─────────────────────────────────────────
    s15e = _common15.sanitize_session_id("sess\x00id")
    test("15e: null bytes stripped from session ID", "\x00" not in s15e,
         f"got: {s15e!r}")

    # ── 15f: Empty/non-string input falls back to 'default-session' ──────────
    test("15f: empty string → 'default-session'",
         _common15.sanitize_session_id("") == "default-session")
    test("15f: None input → 'default-session'",
         _common15.sanitize_session_id(None) == "default-session")  # type: ignore[arg-type]
...
try:
    ...
    test("16a: payload sessionId wins when env vars absent",
         sid16a == "payload-sid-alpha", f"got: {sid16a!r}")
    ...
    test("16b: payload sessionId wins over env COPILOT_AGENT_SESSION_ID",
         sid16b == "payload-wins", f"got: {sid16b!r}")

    # 16c: Without data, env chain still works (backward compat).
    sid16c = _common16.get_session_id()
    test("16c: no data → falls back to env chain (COPILOT_AGENT_SESSION_ID)",
         sid16c == "env-agent-sid", f"got: {sid16c!r}")

    # 16d: Empty-string sessionId in payload is NOT used (treated as absent).
    sid16d = _common16.get_session_id({"sessionId": ""})
    test("16d: empty-string payload sessionId → env chain used",
         sid16d == "env-agent-sid", f"got: {sid16d!r}")

    # 16e: Non-string sessionId in payload is NOT used.
    sid16e = _common16.get_session_id({"sessionId": 12345})
    test("16e: non-string payload sessionId → env chain used",
         sid16e == "env-agent-sid", f"got: {sid16e!r}")
    ...
    try:
        ...
        test("16f: write suffix == cleanup suffix from same data",
             write_suffix == cleanup_suffix, f"write={write_suffix!r} cleanup={cleanup_suffix!r}")
        test("16f: suffix contains payload session id",
             "s16-roundtrip" in write_suffix, f"got: {write_suffix!r}")

        # Verify the state path resolves to the right file.
        state_path16 = _common16.get_session_state_path(_data16)
        test("16f: state path contains payload session id",
             "s16-roundtrip" in str(state_path16), f"got: {state_path16}")
    finally:
...
try:
    ...
    try:
        ...
        test("17b: each concurrent write used a unique temp path",
             unique_tmps == N,
        ...
        test("17c: final state file is valid JSON after concurrent writes",
             "total_tokens" in final_state)
    finally:
...
try:
    ...
    test("18a: '/' in payload sessionId is sanitized", "/" not in suffix18a,
         f"got: {suffix18a!r}")
    test("18a: '\\\\' in payload sessionId is sanitized", "\\" not in suffix18a,
         f"got: {suffix18a!r}")
    ...
    try:
        ...
        try:
            ...
            stays_in_dir = False
        test("18b: state path with traversal sessionId stays in MARKERS_DIR",
             stays_in_dir, f"path={resolved18} dir={resolved_dir18}")
    finally:
    ...
    test("18c: null byte in payload sessionId is stripped",
         "\x00" not in _common18.sanitize_session_id(sid18c))
    ...
    try:
        ...
        test("18d: save/load round-trip via data sessionId succeeds",
             loaded18d.get("total_tokens") == 42, f"got: {loaded18d}")
        ...
        test("18d: state file is written to data-derived path",
             expected_file.is_file(), f"file not found: {expected_file}")
    finally:
...
try:
    ...
    state22.write_text('{"files_read": {}, "total_tokens": 0, "thresholds_warned": []}',
                       encoding="utf-8")
    lock22.write_text("", encoding="utf-8")  # simulate orphaned lock from killed process
    other22.write_text('{"files_read": {}, "total_tokens": 5, "thresholds_warned": []}',
                       encoding="utf-8")
...
try:
    ...
    stale_state23.write_text('{"files_read": {}, "total_tokens": 0, "thresholds_warned": []}',
                              encoding="utf-8")
