...
from browse.components import banner, data_table, page_header
...
def handle_home(db, params, token, nonce) -> tuple:
    ...
    body = (
        ...
        f"</form>\n"
        + banner(
            f'<a href="/dashboard{tok_qs}">View full dashboard</a> for trends, red flags, and most-referenced modules.',
            variant="info",
            icon="👉",
        )
        + page_header("Recent Sessions", subtitle_html=f"Most recent {len(rows)} sessions.")
