...
def test_v8_migration_fresh_db():
    ...
    try:
        result = subprocess.run(
            [sys.executable, str(Path(__file__).parent / "migrate.py"), db_path],
...
def _create_query_session_shim():
    ...
    qs_path = Path(__file__).parent / "query-session.py"
...
def _create_build_session_index_shim():
    ...
    import importlib.util
    bsi_path = Path(__file__).parent / "build-session-index.py"
