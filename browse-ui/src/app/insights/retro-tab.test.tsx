...
import { RetroTab } from "@/app/insights/retro-tab";
...
vi.mock("@/app/insights/retro-section", () => ({
  RetroBody: () => <div data-testid="retro-body" />,
}));

import { useRetro } from "@/lib/api/hooks";
const mockedUseRetro = vi.mocked(useRetro);
...
describe("RetroTab — idle states", () => {
  ...
  it("shows 'No agent host selected' guidance when no host is configured", () => {
    ...
    expect(mockedUseRetro).not.toHaveBeenCalled();
  });
