...
pub fn is_pid_running(pid: u32) -> bool {
    #[cfg(unix)]
    {
        ...
        // EPERM (errno 1) means the process exists but we lack permission to signal it.
        // This is common when the watcher runs as another user.
...
/// `EPERM` errno value (1 on all Unix platforms).
#[cfg(unix)]
const LIBC_EPERM: i32 = 1;
    ...
    fn test_read_db_stats_empty_db() {
        ...
        conn.execute_batch(
            "CREATE TABLE knowledge_entries (id INTEGER PRIMARY KEY);
             CREATE TABLE sessions (id INTEGER PRIMARY KEY, indexed_at TEXT);",
        )
        .unwrap();
        drop(conn);
