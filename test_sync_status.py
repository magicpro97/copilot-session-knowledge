...
test("plain run mentions Sync", "Sync" in r.stdout or "Watcher" in r.stdout,
...
test("--watch-status mentions Lock", "Lock" in r.stdout or "lock" in r.stdout.lower(),
     f"output:\n{r.stdout[:200]}")
...
test("--health-check exits 0 or 2", r.returncode in (0, 2),
     f"exit={r.returncode}\n{r.stderr[:200]}")
test("--health-check mentions DB", "DB" in r.stdout or "db" in r.stdout.lower(),
     f"output:\n{r.stdout[:200]}")
...
try:
    ...
    test("health-check ok is bool", isinstance(hc.get("ok"), bool),
         f"ok={hc.get('ok')!r}")
except json.JSONDecodeError as exc:
...
test("--audit mentions local-db", "local-db" in r.stdout,
     f"output:\n{r.stdout[:200]}")
...
test("--doctor exits 0", r.returncode == 0,
     f"exit={r.returncode}\n{r.stderr[:200]}")
test("--doctor mentions python", "python" in r.stdout.lower(),
     f"output:\n{r.stdout[:300]}")
...
test("--watch-status mentions Lock", "Lock" in r.stdout or "lock" in r.stdout.lower(),
     f"output:\n{r.stdout[:200]}")
test("--watch-status does not crash", r.returncode != 1 or "Traceback" not in r.stderr,
     r.stderr[:200])
...
test("--audit-runtime exits 0 or 2", r.returncode in (0, 2),
     f"exit={r.returncode}\n{r.stderr[:200]}")
test("--audit-runtime mentions local-db", "local-db" in r.stdout,
     f"output:\n{r.stdout[:200]}")
...
test("missing SESSION_STATE exits 1", r.returncode == 1,
     f"exit={r.returncode}")
test("missing SESSION_STATE prints error", "not found" in r.stdout.lower() or "not found" in r.stderr.lower(),
...
test("empty SESSION_STATE --once exits 0", r.returncode == 0,
     f"exit={r.returncode}\nstdout={r.stdout[:200]}\nstderr={r.stderr[:200]}")
test("empty SESSION_STATE --once prints Watching", "[watch]" in r.stdout,
     f"stdout={r.stdout[:200]}")

# 3c: --once writes .watch-state.json after successful run
state_file = Path(home_empty, ".copilot", "session-state", ".watch-state.json")
test("--once creates .watch-state.json", state_file.exists(),
     f"state file not found: {state_file}")
if state_file.exists():
    try:
        state = json.loads(state_file.read_text(encoding="utf-8"))
        test("watch-state.json has signatures key", "signatures" in state,
             f"keys={list(state.keys())}")
        test("watch-state.json has last_index key", "last_index" in state,
             f"keys={list(state.keys())}")
    except json.JSONDecodeError as exc:
...
test("--once cleans up lock file after exit", not lock_file.exists(),
     f"stale lock at {lock_file}")
...
test("stale lock --once exits 0", r.returncode == 0,
     f"exit={r.returncode}\nstdout={r.stdout[:200]}")
test("stale lock prints removal notice", "stale" in r.stdout.lower(),
     f"stdout={r.stdout[:200]}")
test("stale lock cleaned up after run", not (stale_ss / ".watcher.lock").exists(),
     "lock file still present after run")
...
test("live lock conflict exits 1", r.returncode == 1,
     f"exit={r.returncode}\nstdout={r.stdout[:200]}")
test("live lock conflict mentions PID", str(live_pid) in r.stdout,
     f"stdout={r.stdout[:200]}")
...
if _platform.system() == "Windows":
    ...
    test("--install-hint mentions reboot or crontab", "reboot" in r.stdout or "crontab" in r.stdout,
         f"stdout={r.stdout[:200]}")
...
test("--once with session files exits 0", r.returncode == 0,
     f"exit={r.returncode}\nstdout={r.stdout[:200]}")
...
if wf_state_file.exists():
    try:
        ...
        test("--once records .md file in signatures", found_md,
             f"signatures keys: {list(sigs.keys())[:5]}")
    except json.JSONDecodeError as exc:
